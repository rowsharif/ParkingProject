...
const CRUDhistories = (props) => {
  ...
  useEffect(() => {
    ...
    db.collection("History").doc(firebase.auth().currentUser.uid).onSnapshot(querySnapshot => {
    const history = [""];
    querySnapshot.forEach(doc => {
 ...
 {histories.map((history,i)=>(
    ...
    borderWidth: 1,}}>
      <Text>
  {"Total Amount - "+ history.TotalAmount+"QR,      Car - "+ history.CarId + " ,   Parking-  "+ history.ParkingId+ " ,   Duration-  "+history.Duration+" ,   DateTime-  "+history.Date(DateTime.toString())+"\n"}</Text></View>
 ))}     
